...
PREDICTOR_TO_COLOUR_MAP_DICT = {
    image_utils.TEMPERATURE_NAME: pyplot.get_cmap('YlOrRd'),
    image_utils.REFLECTIVITY_NAME: REFL_COLOUR_MAP_OBJECT,
    image_utils.U_WIND_NAME: pyplot.get_cmap('seismic'),
    image_utils.V_WIND_NAME: pyplot.get_cmap('seismic')
}
...
def plot_many_predictors_with_barbs(
    ...
    non_wind_predictor_names = [
        ...
        if p not in [image_utils.U_WIND_NAME, image_utils.V_WIND_NAME]
    ]
    ...
    for k in range(len(non_wind_predictor_names)):
        ...
        if non_wind_predictor_names[k] == image_utils.REFLECTIVITY_NAME:
...
def plot_many_predictors_sans_barbs(
    ...
    for i in range(num_panel_rows):
        for j in range(num_panel_columns):
            ...
            if predictor_names[k] == image_utils.REFLECTIVITY_NAME:
                this_colour_norm_object = REFL_COLOUR_NORM_OBJECT
                this_min_colour_value = None
                this_max_colour_value = None
            elif predictor_names[k] == image_utils.TEMPERATURE_NAME:
                this_colour_norm_object = None
                this_min_colour_value = min_colour_temp_kelvins + 0.
                this_max_colour_value = max_colour_temp_kelvins + 0.
            else:
